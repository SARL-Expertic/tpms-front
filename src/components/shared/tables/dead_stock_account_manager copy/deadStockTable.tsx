...
import { FETCH_ALL_DEAD_STOCK } from "@/app/api/tickets"
import NewDeadStockModal from "../../modal/deadstock_manager/Nouvelle_DEADSTOCK"
import DeadStockSummary from "./DeadStockSummary"
...
// Filter configuration for dead stock table
const deadStockFilters = [
  ...
  },
  {
    ...
    labelMap: (value: string) => getConditionLabel(value), // Convert to French
  },
  {
    key: "bankname" as keyof DeadStock,
    label: "Banque",
...
export default function DeadStockTable() {
  ...
  // Function to fetch and refresh dead stock items
  const fetchAndSetDeadStock = () => {
    setLoading(true)
    FETCH_ALL_DEAD_STOCK()
      .then((res) => {
        const items = res.data || []
        // Map items to add bankname field for filtering
        const mappedItems = items.map((item: DeadStock) => ({
          ...item,
          bankname: item.bank?.name || 'Non assigné'
        }))
        setDeadStockItems(mappedItems)
        setLoading(false)
      })
      .catch((err) => {
    ...
    <div className="mx-auto py-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-blue-800 
          ...
          Dead Stock - Inventaire
        </h1>
        <div className="flex gap-3 items-center">
          <NewDeadStockModal onSuccess={() => {
            fetchAndSetDeadStock()
            showNotification('success', 'Article dead stock créé avec succès!')
          }} />
        </div>
      </div>

      {/* Summary Statistics */}
      {!loading && !error && <DeadStockSummary />}
