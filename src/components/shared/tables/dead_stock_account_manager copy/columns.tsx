...
import { DeadStockDetailsButton } from "../../modal/deadstock_manager/DEADSTOCKdetailsButton";
import { DeadStockDeleteButton } from "../../modal/deadstock_manager/DELETEEdetailsButtoN";
import { getConditionLabel } from "@/constants/deadstock/conditions";

// Columns for dead stock items
    ...
    cell: ({ row }) => (
      row.original.bank ? (
        <div className="flex flex-col">
          <span className="font-medium">{row.original.bank.name}</span>
        </div>
      ) : (
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex gap-2">
        <DeadStockDetailsButton
          deadStock={row.original}
          onSave={onRefresh}
        />
        <DeadStockDeleteButton
          deadStock={row.original}
          onDelete={onRefresh}
        />
      </div>
